...
function createWithdrawItem(item) {
  ...
  money.className = `history__article ${
    item.type === "income" ? "income" : "expend"
  }`;
  money.textContent =
    item.type === "income"
      ? `+ ${item.money.toLocaleString()}`
      : `- ${Math.abs(item.money).toLocaleString()}`;
...
function renderWithdrawList(filteredHistory) {
  ...
  const totalExpend = filterExpend.reduce(
    (total, item) => total + item.money,
    0
  );
...
document.querySelector(".submitBtn").addEventListener("click", function () {
  ...
  const money = parseInt(
    document.querySelector(".moneyInput").value.replaceAll(",", "")
  );
